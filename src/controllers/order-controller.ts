...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (!['delivery', 'pickup'].includes(deliveryMethod)) {
      throwBadRequest("Invalid delivery method. Must be 'delivery' or 'pickup'")
    }
    ...
    await db.transaction(async (tx) => {
      ...
      if (promoCodeId) {
        const [promo] = await tx.select().from(promoCodes).where(eq(promoCodes._id, promoCodeId))
...
export const getOrderById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const allProducts = await db.select().from(products).where(inArray(products._id, productIds))
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      throwBadRequest(`Invalid status. Must be one of: ${validStatuses.join(', ')}`)
    }

    const [existingOrder] = await db.select().from(orders).where(eq(orders._id, id))
    ...
    if (updatedOrder.status === 'delivered') {
      eventBus.emit('confirmationEmail', { orderId: updatedOrder._id })
    }
...
// ERROR HANDLER
// ------------------------
function handleControllerError(error: unknown, res: Response, fallbackMsg: string) {

...
export const createPromo = async (req: Request, res: Response) => {
  try {
    const { name, code, discount, isPercentage, description, ctaText, subtitle, urgent } = req.body

    if (!name || !code || discount === undefined || !description || !ctaText || !subtitle) {
...
export const editCodeDetails = async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    const { name, code, discount, isPercentage, isActive, description, ctaText, subtitle, urgent } =
      req.body
...
export const getSinglePromo = async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.select().from(promoCodes).where(eq(promoCodes._id, id)).limit(1)

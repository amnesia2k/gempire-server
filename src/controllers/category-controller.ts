...
export const safeInvalidateCategory = async (categoryId: string | null | undefined) => {
...
export const createCategory = async (req: Request, res: Response) => {
  try {
    ...
    const [existing] = await db.select().from(category).where(eq(category.slug, slug))

    if (existing) {
      throw throwBadRequest(`Category "${name}" already exists. Try a different name.`)
    }
    ...
    const [inserted] = await db.insert(category).values(newCategory).returning()
